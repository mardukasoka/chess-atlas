...
describe("Ludus Latrunculorum — Schädler 1994 reconstruction", () => {
  ...
  test("alternating placement leads into movement after both armies are placed", () => {
    ...
    game.place(0, 0); game.place(3, 3); game.place(0, 1); game.place(3, 2);
  ...
  test("movement is one empty orthogonal square", () => {
    ...
    game.set(2, 2, "black"); game.set(2, 3, "white"); game.refreshTraps();
  ...
  test("a piece can make a multiple orthogonal leap over occupied pieces", () => {
    const game = movementGame({ rows: 5, cols: 5 });
    game.set(2, 0, "black"); game.set(2, 1, "white"); game.set(2, 3, "black"); game.refreshTraps();
  ...
  test("custodial enclosure immobilises the surrounded piece", () => {
    ...
    game.set(3, 2, "black"); game.set(3, 3, "white"); game.set(3, 4, "black"); game.refreshTraps();
  ...
  test("an enclosed piece is freed immediately if one enclosing enemy is itself surrounded", () => {
    const game = movementGame({ rows: 7, cols: 7 });
    game.set(3, 3, "white");
    game.set(3, 2, "black");
    game.set(3, 4, "black");
    game.set(2, 4, "white");
    game.set(4, 4, "white");
    game.refreshTraps();
    expect(game.isTrapped(3, 4)).toBe(true);
    expect(game.isTrapped(3, 3)).toBe(false);
  });

  test("an enclosed piece is captured only when the enclosing player gets the next turn", () => {
    const game = movementGame({ rows: 6, cols: 6 });
    game.placed = { black: 3, white: 3 };
    game.set(3, 2, "black"); game.set(2, 4, "black"); game.set(5, 5, "black");
    game.set(3, 3, "white"); game.set(0, 0, "white"); game.set(5, 0, "white"); game.refreshTraps();
    game.move(2, 4, 3, 4);
