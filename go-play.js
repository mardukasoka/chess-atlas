"use strict";
(()=>{
  const Go=window.ChessAtlasGoModule,Modules=window.ChessAtlasGameModules,Agents=window.ChessAtlasGameAgents,Heuristics=window.ChessAtlasAgentHeuristics,boardEl=document.getElementById('go-board'),status=document.getElementById('go-status'),detail=document.getElementById('go-detail');
  ...
  function opponentMode(){return document.getElementById('go-opponent').value;}
  function activeAgent(){const mode=opponentMode();if(mode==='random')return Agents?.randomAgent({id:'go-random',name:'Random legal'});if(mode==='heuristic')return Heuristics?.createAgent(Agents,'go',{id:'go-heuristic',name:'Atlas heuristic'});return null;}
  function isAgentTurn(){const snap=Go.snapshot(game);return opponentMode()!=='human'&&snap.status==='playing'&&snap.turn==='white';}
  ...
  async function maybeAgent(){const agent=activeAgent();if(!isAgentTurn()||busy||!agent||!Modules)return;busy=true;render();try{const result=await Agents.takeTurn({modules:Modules,gameId:'go',game,agent});if(result.status==='applied')game=result.result;}catch(e){status.textContent=`Agent error: ${e.message}`;}finally{busy=false;render();}}
