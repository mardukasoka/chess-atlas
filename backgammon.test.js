...
describe('Backgammon topology and dice sequencing',()=>{
  ...
  test('if only one of two dice can be used the higher die is required',()=>{let s=B.create();s.status='playing';s.turn='white';s.points=Array(25).fill(0);s.bar.white=1;s.off.white=14;s.points[22]=-2;s.off.black=13;s.diceRemaining=[1,2];const moves=B.legalNextMoves(s);expect(moves.length).toBe(1);expect(moves[0].from).toBe('bar');expect(moves[0].die).toBe(2);expect(moves[0].to).toBe(23);});
});
